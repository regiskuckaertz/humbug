...
case class Podcast(linkUrl: String, copyright: String, author: String, subscriptionUrl: Option[String] = None, explicit: Boolean, image: Option[String] = None, categories: Option[List[PodcastCategory]] = None, podcastType: Option[String] = None) extends TStruct
...
object Podcast extends TStructCodec[Podcast] {
  ...
  override val defaults = HMap[TFieldCodec](w4.value -> None, w6.value -> None, w7.value -> None, w8.value -> None)

  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.linkUrl, w2.value -> x.copyright, w3.value -> x.author, w4.value -> x.subscriptionUrl, w5.value -> x.explicit, w6.value -> x.image, w7.value -> x.categories, w8.value -> x.podcastType)
  ...
  override def decode = (m) ⇒ for {
    ...
    podcastType ← m.get(w8.value).orElse(defaults.get(w8.value))
  } yield Podcast(linkUrl, copyright, author, subscriptionUrl, explicit, image, categories, podcastType)
}