...
case class StoriesResponse(status: String, total: Int, results: List[Story]) extends TStruct

object StoriesResponse extends TStructCodec[StoriesResponse] {
  val w1 = Witness(1)
  ...
  implicit val r3 = new TFieldCodec[w3.T, List[Story]]()
  ...
  } yield StoriesResponse(status, total, results)
}