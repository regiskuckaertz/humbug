...
case class Block(id: String, bodyHtml: String, bodyTextSummary: String, title: Option[String] = None, attributes: BlockAttributes, published: Boolean, createdDate: Option[CapiDateTime] = None, firstPublishedDate: Option[CapiDateTime] = None, publishedDate: Option[CapiDateTime] = None, lastModifiedDate: Option[CapiDateTime] = None, contributors: List[String], createdBy: Option[User] = None, lastModifiedBy: Option[User] = None, elements: List[BlockElement] = List()) extends TStruct
...
object Block extends TStructCodec[Block] {
  ...
  override val defaults = HMap[TFieldCodec](w4.value -> None, w7.value -> None, w8.value -> None, w9.value -> None, w10.value -> None, w12.value -> None, w13.value -> None, w14.value -> List())

  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.id, w2.value -> x.bodyHtml, w3.value -> x.bodyTextSummary, w4.value -> x.title, w5.value -> x.attributes, w6.value -> x.published, w7.value -> x.createdDate, w8.value -> x.firstPublishedDate, w9.value -> x.publishedDate, w10.value -> x.lastModifiedDate, w11.value -> x.contributors, w12.value -> x.createdBy, w13.value -> x.lastModifiedBy, w14.value -> x.elements)
  ...
  } yield Block(id, bodyHtml, bodyTextSummary, title, attributes, published, createdDate, firstPublishedDate, publishedDate, lastModifiedDate, contributors, createdBy, lastModifiedBy, elements)
}