...
case class StandardElementFields(url: Option[String] = None, originalUrl: Option[String] = None, source: Option[String] = None, title: Option[String] = None, description: Option[String] = None, credit: Option[String] = None, caption: Option[String] = None, width: Option[Int] = None, height: Option[Int] = None, html: Option[String] = None, role: Option[String] = None, isMandatory: Option[Boolean] = None) extends TStruct
...
object StandardElementFields extends TStructCodec[StandardElementFields] {
  ...
  override val defaults = HMap[TFieldCodec](w1.value -> None, w2.value -> None, w3.value -> None, w4.value -> None, w5.value -> None, w6.value -> None, w7.value -> None, w8.value -> None, w9.value -> None, w10.value -> None, w11.value -> None, w12.value -> None)

  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.url, w2.value -> x.originalUrl, w3.value -> x.source, w4.value -> x.title, w5.value -> x.description, w6.value -> x.credit, w7.value -> x.caption, w8.value -> x.width, w9.value -> x.height, w10.value -> x.html, w11.value -> x.role, w12.value -> x.isMandatory)
  ...
  } yield StandardElementFields(url, originalUrl, source, title, description, credit, caption, width, height, html, role, isMandatory)
}