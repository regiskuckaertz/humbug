...
object WitnessElementFields extends TStructCodec[WitnessElementFields] {
  ...
  override val defaults = HMap[TFieldCodec](w1.value -> None, w2.value -> None, w3.value -> None, w4.value -> None, w5.value -> None, w6.value -> None, w7.value -> None, w8.value -> None, w9.value -> None, w10.value -> None, w11.value -> None, w12.value -> None, w13.value -> None, w14.value -> None, w15.value -> None, w16.value -> None, w17.value -> None, w18.value -> None, w19.value -> None, w20.value -> None, w21.value -> None, w22.value -> None, w23.value -> None, w24.value -> None, w25.value -> None, w26.value -> None)

  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.url, w2.value -> x.originalUrl, w3.value -> x.witnessEmbedType, w4.value -> x.mediaId, w5.value -> x.source, w6.value -> x.title, w7.value -> x.description, w8.value -> x.authorName, w9.value -> x.authorUsername, w10.value -> x.authorWitnessProfileUrl, w11.value -> x.authorGuardianProfileUrl, w12.value -> x.caption, w13.value -> x.alt, w14.value -> x.width, w15.value -> x.height, w16.value -> x.html, w17.value -> x.apiUrl, w18.value -> x.photographer, w19.value -> x.dateCreated, w20.value -> x.youtubeUrl, w21.value -> x.youtubeSource, w22.value -> x.youtubeTitle, w23.value -> x.youtubeDescription, w24.value -> x.youtubeAuthorName, w25.value -> x.youtubeHtml, w26.value -> x.role)
  ...
  } yield WitnessElementFields(url, originalUrl, witnessEmbedType, mediaId, source, title, description, authorName, authorUsername, authorWitnessProfileUrl, authorGuardianProfileUrl, caption, alt, width, height, html, apiUrl, photographer, dateCreated, youtubeUrl, youtubeSource, youtubeTitle, youtubeDescription, youtubeAuthorName, youtubeHtml, role)
}