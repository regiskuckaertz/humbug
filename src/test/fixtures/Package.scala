...
case class Package(packageId: String, articles: List[PackageArticle], packageName: String, lastModified: CapiDateTime) extends TStruct
...
object Package extends TStructCodec[Package] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.packageId, w2.value -> x.articles, w3.value -> x.packageName, w4.value -> x.lastModified)
  ...
  } yield Package(packageId, articles, packageName, lastModified)
}