...
case class Element(id: String, relation: String, `type`: ElementType, galleryIndex: Option[Int] = None, assets: List[Asset]) extends TStruct
...
object Element extends TStructCodec[Element] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.id, w2.value -> x.relation, w3.value -> x.`type`, w4.value -> x.galleryIndex, w5.value -> x.assets)
  ...
  } yield Element(id, relation, `type`, galleryIndex, assets)
}