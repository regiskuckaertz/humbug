...
case class Asset(`type`: AssetType,mimeType: Option[String]= None,file: Option[String]= None,typeData: Option[AssetFields]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.`type`,w2.value -> x.mimeType,w3.value -> x.file,w4.value -> x.typeData)
...
} yield Asset(`type`,mimeType,file,typeData)
}