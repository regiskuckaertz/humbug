...
case class AtomUsageResponse(status: String,userTier: String,total: Int,startIndex: Int,pageSize: Int,currentPage: Int,pages: Int,results: List[String]) extends TStruct

object AtomUsageResponse extends TStructCodec[AtomUsageResponse]{
val w1 = Witness(1)
...
implicit val r8 = new TFieldCodec[w8.T,List[String]]()

override val defaults = HMap[TFieldCodec]()

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.status,w2.value -> x.userTier,w3.value -> x.total,w4.value -> x.startIndex,w5.value -> x.pageSize,w6.value -> x.currentPage,w7.value -> x.pages,w8.value -> x.results)
...
results <- m.get(w8.value).orElse(defaults.get(w8.value))
} yield AtomUsageResponse(status,userTier,total,startIndex,pageSize,currentPage,pages,results)
}