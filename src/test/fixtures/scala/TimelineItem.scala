...
case class TimelineItem(title: String,date: DateTime,body: Option[String]= None,entities: Option[List[Entity]]= None,dateFormat: Option[String]= None,toDate: Option[DateTime]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w3.value -> None,w4.value -> None,w5.value -> None,w6.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.title,w2.value -> x.date,w3.value -> x.body,w4.value -> x.entities,w5.value -> x.dateFormat,w6.value -> x.toDate)
...
} yield TimelineItem(title,date,body,entities,dateFormat,toDate)
}