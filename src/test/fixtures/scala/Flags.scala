...
case class Flags(legallySensitive: Option[Boolean]= None,blockAds: Option[Boolean]= None,sensitive: Option[Boolean]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.legallySensitive,w2.value -> x.blockAds,w3.value -> x.sensitive)
...
} yield Flags(legallySensitive,blockAds,sensitive)
}