...
case class AudioElementFields(
  ...
  explicit:        Option[Boolean] = None) extends TStruct
...
object AudioElementFields extends TStructCodec[AudioElementFields] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.html,
    w2.value -> x.source,
    w3.value -> x.description,
    w4.value -> x.title,
    w5.value -> x.credit,
    w6.value -> x.caption,
    w7.value -> x.durationMinutes,
    w8.value -> x.durationSeconds,
    w9.value -> x.clean,
    w10.value -> x.explicit)
