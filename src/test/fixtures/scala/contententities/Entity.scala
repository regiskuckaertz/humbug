...
case class Entity(id: String,entityType: EntityType,googleId: Option[String]= None,person: Option[Person]= None,film: Option[Film]= None,game: Option[Game]= None,restaurant: Option[Restaurant]= None,place: Option[Place]= None,organisation: Option[Organisation]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w3.value -> None,w4.value -> None,w5.value -> None,w6.value -> None,w7.value -> None,w8.value -> None,w9.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.id,w2.value -> x.entityType,w3.value -> x.googleId,w4.value -> x.person,w5.value -> x.film,w6.value -> x.game,w7.value -> x.restaurant,w8.value -> x.place,w9.value -> x.organisation)
...
} yield Entity(id,entityType,googleId,person,film,game,restaurant,place,organisation)
}