...
case class Game(title: String,publisher: Option[String]= None,platforms: List[String],price: Option[Price]= None,pegiRating: Option[Int]= None,genre: List[String]) extends TStruct
...
override val defaults = HMap[TFieldCodec](w2.value -> None,w4.value -> None,w5.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.title,w2.value -> x.publisher,w3.value -> x.platforms,w4.value -> x.price,w5.value -> x.pegiRating,w6.value -> x.genre)
