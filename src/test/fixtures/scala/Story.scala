...
case class Story(id: String,name: String,summary: String,events: List[StoryEvent],atoms: List[Atom]) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.id,w2.value -> x.name,w3.value -> x.summary,w4.value -> x.events,w5.value -> x.atoms)
...
} yield Story(id,name,summary,events,atoms)
}