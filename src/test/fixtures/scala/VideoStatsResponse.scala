...
case class VideoStatsResponse(status: String,mostViewedVideos: List[MostViewedVideo]) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.status,w2.value -> x.mostViewedVideos)

override def decode = (m) => for {
status <- m.get(w1.value).orElse(defaults.get(w1.value))

mostViewedVideos <- m.get(w2.value).orElse(defaults.get(w2.value))
} yield VideoStatsResponse(status,mostViewedVideos)
}