...
case class Sponsorship(
  ...
  sponsorLogo:                       String,
  sponsorLink:                       String,
  targeting:                         Option[SponsorshipTargeting]      = None,
  aboutLink:                         Option[String]                    = None,
  sponsorLogoDimensions:             Option[SponsorshipLogoDimensions] = None,
  highContrastSponsorLogo:           Option[String]                    = None,
  highContrastSponsorLogoDimensions: Option[SponsorshipLogoDimensions] = None) extends TStruct
...
object Sponsorship extends TStructCodec[Sponsorship] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.sponsorshipType,
    w2.value -> x.sponsorName,
    w3.value -> x.sponsorLogo,
    w4.value -> x.sponsorLink,
    w5.value -> x.targeting,
    w6.value -> x.aboutLink,
    w7.value -> x.sponsorLogoDimensions,
    w8.value -> x.highContrastSponsorLogo,
    w9.value -> x.highContrastSponsorLogoDimensions)
