...
override val defaults = HMap[TFieldCodec](w5.value -> None,w6.value -> None,w7.value -> None,w8.value -> None,w9.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.sponsorshipType,w2.value -> x.sponsorName,w3.value -> x.sponsorLogo,w4.value -> x.sponsorLink,w5.value -> x.targeting,w6.value -> x.aboutLink,w7.value -> x.sponsorLogoDimensions,w8.value -> x.highContrastSponsorLogo,w9.value -> x.highContrastSponsorLogoDimensions)
...
} yield Sponsorship(sponsorshipType,sponsorName,sponsorLogo,sponsorLink,targeting,aboutLink,sponsorLogoDimensions,highContrastSponsorLogo,highContrastSponsorLogoDimensions)
}