...
case class ContentAtomElementFields(atomId: String,atomType: String) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.atomId,w2.value -> x.atomType)

override def decode = (m) => for {
atomId <- m.get(w1.value).orElse(defaults.get(w1.value))

atomType <- m.get(w2.value).orElse(defaults.get(w2.value))
} yield ContentAtomElementFields(atomId,atomType)
}