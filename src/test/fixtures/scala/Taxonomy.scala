...
case class Taxonomy(tags: Option[List[TagUsage]]= None,contributors: Option[List[Tag]]= None,publication: Option[Tag]= None,newspaper: Option[Newspaper]= None,references: Option[List[Reference]]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.tags,w2.value -> x.contributors,w3.value -> x.publication,w4.value -> x.newspaper,w5.value -> x.references)
...
} yield Taxonomy(tags,contributors,publication,newspaper,references)
}