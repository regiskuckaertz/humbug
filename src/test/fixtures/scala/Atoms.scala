...
case class Atoms(
  quizzes:        Option[List[Atom]] = None,
  viewpoints:     Option[List[Atom]] = None,
  media:          Option[List[Atom]] = None,
  explainers:     Option[List[Atom]] = None,
  cta:            Option[List[Atom]] = None,
  interactives:   Option[List[Atom]] = None,
  reviews:        Option[List[Atom]] = None,
  recipes:        Option[List[Atom]] = None,
  storyquestions: Option[List[Atom]] = None,
  qandas:         Option[List[Atom]] = None,
  guides:         Option[List[Atom]] = None,
  profiles:       Option[List[Atom]] = None,
  timelines:      Option[List[Atom]] = None) extends TStruct
...
object Atoms extends TStructCodec[Atoms] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.quizzes,
    w2.value -> x.viewpoints,
    w3.value -> x.media,
    w4.value -> x.explainers,
    w5.value -> x.cta,
    w6.value -> x.interactives,
    w7.value -> x.reviews,
    w8.value -> x.recipes,
    w9.value -> x.storyquestions,
    w10.value -> x.qandas,
    w11.value -> x.guides,
    w12.value -> x.profiles,
    w13.value -> x.timelines)
