...
case class CommentElementFields(
  source:        Option[String] = None,
  discussionKey: Option[String] = None,
  commentUrl:    Option[String] = None,
  originalUrl:   Option[String] = None,
  sourceUrl:     Option[String] = None,
  discussionUrl: Option[String] = None,
  authorUrl:     Option[String] = None,
  html:          Option[String] = None,
  authorName:    Option[String] = None,
  commentId:     Option[Int]    = None) extends TStruct
...
object CommentElementFields extends TStructCodec[CommentElementFields] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.source,
    w2.value -> x.discussionKey,
    w3.value -> x.commentUrl,
    w4.value -> x.originalUrl,
    w5.value -> x.sourceUrl,
    w6.value -> x.discussionUrl,
    w7.value -> x.authorUrl,
    w8.value -> x.html,
    w9.value -> x.authorName,
    w10.value -> x.commentId)
