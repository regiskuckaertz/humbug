...
case class Answer(answerId: String,answerType: AnswerType) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.answerId,w2.value -> x.answerType)

override def decode = (m) => for {
answerId <- m.get(w1.value).orElse(defaults.get(w1.value))

answerType <- m.get(w2.value).orElse(defaults.get(w2.value))
} yield Answer(answerId,answerType)
}