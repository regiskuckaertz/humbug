...
case class InteractiveAtom(`type`: String,title: String,css: String,html: String,mainJS: Option[String]= None,docData: Option[String]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w5.value -> None,w6.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.`type`,w2.value -> x.title,w3.value -> x.css,w4.value -> x.html,w5.value -> x.mainJS,w6.value -> x.docData)
...
} yield InteractiveAtom(`type`,title,css,html,mainJS,docData)
}