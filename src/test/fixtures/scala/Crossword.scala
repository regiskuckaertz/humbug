...
case class Crossword(
  ...
  creator:               Option[CrosswordCreator] = None,
  pdf:                   Option[String]           = None,
  annotatedSolution:     Option[String]           = None,
  dateSolutionAvailable: Option[CapiDateTime]     = None) extends TStruct
...
object Crossword extends TStructCodec[Crossword] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    ...
    w3.value -> x.number,
    w4.value -> x.date,
    w5.value -> x.dimensions,
    w6.value -> x.entries,
    w7.value -> x.solutionAvailable,
    w8.value -> x.hasNumbers,
    w9.value -> x.randomCluesOrdering,
    w10.value -> x.instructions,
    w11.value -> x.creator,
    w12.value -> x.pdf,
    w13.value -> x.annotatedSolution,
    w14.value -> x.dateSolutionAvailable)
