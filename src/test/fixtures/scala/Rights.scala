...
case class Rights(syndicatable: Option[Boolean]= Some(`false`),subscriptionDatabases: Option[Boolean]= Some(`false`),developerCommunity: Option[Boolean]= Some(`false`)) extends TStruct
...
override val defaults = HMap[TFieldCodec](w1.value -> Some(`false`),w2.value -> Some(`false`),w3.value -> Some(`false`))

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.syndicatable,w2.value -> x.subscriptionDatabases,w3.value -> x.developerCommunity)
