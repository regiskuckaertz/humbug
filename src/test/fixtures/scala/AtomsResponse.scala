...
case class AtomsResponse(status: String,userTier: String,total: Int,startIndex: Int,pageSize: Int,currentPage: Int,pages: Int,results: List[Atom]) extends TStruct

object AtomsResponse extends TStructCodec[AtomsResponse]{
val w1 = Witness(1)
...
implicit val r8 = new TFieldCodec[w8.T,List[Atom]]()
...
} yield AtomsResponse(status,userTier,total,startIndex,pageSize,currentPage,pages,results)
}