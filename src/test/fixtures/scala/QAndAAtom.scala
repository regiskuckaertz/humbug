...
case class QAndAAtom(typeLabel: Option[String]= None,eventImage: Option[Image]= None,item: QAndAItem,question: Option[Question]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.typeLabel,w2.value -> x.eventImage,w3.value -> x.item,w4.value -> x.question)
...
} yield QAndAAtom(typeLabel,eventImage,item,question)
}