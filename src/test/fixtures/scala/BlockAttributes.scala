...
case class BlockAttributes(keyEvent: Option[Boolean]= None,summary: Option[Boolean]= None,title: Option[String]= None,pinned: Option[Boolean]= None,membershipPlaceholder: Option[MembershipPlaceholder]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.keyEvent,w2.value -> x.summary,w3.value -> x.title,w4.value -> x.pinned,w5.value -> x.membershipPlaceholder)
...
} yield BlockAttributes(keyEvent,summary,title,pinned,membershipPlaceholder)
}