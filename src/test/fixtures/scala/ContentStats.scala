...
object ContentStats extends TStructCodec[ContentStats] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    ...
    w3.value -> x.text,
    w4.value -> x.tweets,
    w5.value -> x.pullquotes,
    w6.value -> x.audio,
    w7.value -> x.interactives,
    w8.value -> x.witness,
    w9.value -> x.richlinks,
    w10.value -> x.membership,
    w11.value -> x.embeds,
    w12.value -> x.comments,
    w13.value -> x.instagram,
    w14.value -> x.vines)
