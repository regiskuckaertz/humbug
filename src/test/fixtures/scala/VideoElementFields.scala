...
case class VideoElementFields(
  ...
  originalUrl:               Option[String]  = None) extends TStruct
...
object VideoElementFields extends TStructCodec[VideoElementFields] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    ...
    w8.value -> x.height,
    w9.value -> x.width,
    w10.value -> x.duration,
    w11.value -> x.contentAuthSystem,
    w12.value -> x.embeddable,
    w13.value -> x.isInappropriateForAdverts,
    w14.value -> x.mediaId,
    w15.value -> x.stillImageUrl,
    w16.value -> x.thumbnailUrl,
    w17.value -> x.shortUrl,
    w18.value -> x.role,
