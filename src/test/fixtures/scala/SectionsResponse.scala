...
case class SectionsResponse(status: String,userTier: String,total: Int,results: List[Section]) extends TStruct

object SectionsResponse extends TStructCodec[SectionsResponse]{
val w1 = Witness(1)
...
implicit val r4 = new TFieldCodec[w4.T,List[Section]]()
...
} yield SectionsResponse(status,userTier,total,results)
}