...
case class VineElementFields(
  ...
  caption:        Option[String] = None) extends TStruct
...
object VineElementFields extends TStructCodec[VineElementFields] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.originalUrl,
    w2.value -> x.title,
    w3.value -> x.source,
    w4.value -> x.authorUrl,
    w5.value -> x.authorUsername,
    w6.value -> x.html,
    w7.value -> x.width,
    w8.value -> x.height,
    w9.value -> x.alt,
