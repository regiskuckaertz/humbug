...
object ImageElementFields extends TStructCodec[ImageElementFields] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.caption,
    w2.value -> x.copyright,
    w3.value -> x.displayCredit,
    w4.value -> x.credit,
    w5.value -> x.source,
    w6.value -> x.photographer,
    w7.value -> x.alt,
    w8.value -> x.mediaId,
    w9.value -> x.mediaApiUri,
    w10.value -> x.picdarUrn,
    w11.value -> x.suppliersReference,
    w12.value -> x.imageType,
    w13.value -> x.comment,
    w14.value -> x.role)
