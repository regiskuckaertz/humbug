...
case class Tag(
  ...
  activeSponsorships:        Option[List[Sponsorship]] = None,
  paidContentType:           Option[String]            = None,
  paidContentCampaignColour: Option[String]            = None,
  rcsId:                     Option[String]            = None,
  r2ContributorId:           Option[String]            = None,
  tagCategories:             Option[Set[String]]       = None,
  entityIds:                 Option[Set[String]]       = None) extends TStruct
...
object Tag extends TStructCodec[Tag] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    ...
    w6.value -> x.webUrl,
    w7.value -> x.apiUrl,
    w8.value -> x.references,
    w9.value -> x.description,
    w10.value -> x.bio,
    w11.value -> x.bylineImageUrl,
    w12.value -> x.bylineLargeImageUrl,
    w13.value -> x.podcast,
    w14.value -> x.firstName,
    w15.value -> x.lastName,
    w16.value -> x.emailAddress,
    w17.value -> x.twitterHandle,
    w18.value -> x.activeSponsorships,
    w19.value -> x.paidContentType,
    w20.value -> x.paidContentCampaignColour,
    w21.value -> x.rcsId,
    w22.value -> x.r2ContributorId,
    w23.value -> x.tagCategories,
    w24.value -> x.entityIds)
