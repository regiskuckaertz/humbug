...
case class ResultBucket(assets: Option[List[Asset]]= None,description: String,title: String,share: String,id: String) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.assets,w2.value -> x.description,w3.value -> x.title,w4.value -> x.share,w5.value -> x.id)
...
} yield ResultBucket(assets,description,title,share,id)
}