...
case class Article(id: String,articleType: ArticleType,group: Group,headline: Option[String]= None,href: Option[String]= None,trailText: Option[String]= None,imageSrc: Option[String]= None,isBoosted: Option[Boolean]= None,imageHide: Option[Boolean]= None,showMainVideo: Option[Boolean]= None,showKickerTag: Option[Boolean]= None,showKickerSection: Option[Boolean]= None,byline: Option[String]= None,customKicker: Option[String]= None,showBoostedHeadline: Option[Boolean]= None,showQuotedHeadline: Option[Boolean]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w4.value -> None,w5.value -> None,w6.value -> None,w7.value -> None,w8.value -> None,w9.value -> None,w10.value -> None,w11.value -> None,w12.value -> None,w13.value -> None,w14.value -> None,w15.value -> None,w16.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.id,w2.value -> x.articleType,w3.value -> x.group,w4.value -> x.headline,w5.value -> x.href,w6.value -> x.trailText,w7.value -> x.imageSrc,w8.value -> x.isBoosted,w9.value -> x.imageHide,w10.value -> x.showMainVideo,w11.value -> x.showKickerTag,w12.value -> x.showKickerSection,w13.value -> x.byline,w14.value -> x.customKicker,w15.value -> x.showBoostedHeadline,w16.value -> x.showQuotedHeadline)
...
} yield Article(id,articleType,group,headline,href,trailText,imageSrc,isBoosted,imageHide,showMainVideo,showKickerTag,showKickerSection,byline,customKicker,showBoostedHeadline,showQuotedHeadline)
}