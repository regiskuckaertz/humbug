...
case class CrosswordEntry(
  ...
  humanNumber:        Option[String]                 = None,
  direction:          Option[String]                 = None,
  position:           Option[CrosswordPosition]      = None,
  separatorLocations: Option[Map[String, List[Int]]] = None,
  length:             Option[Int]                    = None,
  clue:               Option[String]                 = None,
  group:              Option[List[String]]           = None,
  solution:           Option[String]                 = None,
  format:             Option[String]                 = None) extends TStruct
...
object CrosswordEntry extends TStructCodec[CrosswordEntry] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.id,
    w2.value -> x.number,
    w3.value -> x.humanNumber,
    w4.value -> x.direction,
    w5.value -> x.position,
    w6.value -> x.separatorLocations,
    w7.value -> x.length,
    w8.value -> x.clue,
    w9.value -> x.group,
    w10.value -> x.solution,
    w11.value -> x.format)
