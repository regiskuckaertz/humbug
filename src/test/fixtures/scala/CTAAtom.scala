...
case class CTAAtom(url: String,backgroundImage: Option[String]= None,btnText: Option[String]= None,label: Option[String]= None,trackingCode: Option[String]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.url,w2.value -> x.backgroundImage,w3.value -> x.btnText,w4.value -> x.label,w5.value -> x.trackingCode)
...
} yield CTAAtom(url,backgroundImage,btnText,label,trackingCode)
}