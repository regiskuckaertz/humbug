...
case class EmailProvider(name: String= "xact-targe",listId: String) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.name,w2.value -> x.listId)

override def decode = (m) => for {
name <- m.get(w1.value).orElse(defaults.get(w1.value))

listId <- m.get(w2.value).orElse(defaults.get(w2.value))
} yield EmailProvider(name,listId)
}