...
case class ContentChangeDetails(lastModified: Option[ChangeRecord]= None,created: Option[ChangeRecord]= None,published: Option[ChangeRecord]= None,revision: Long,takenDown: Option[ChangeRecord]= None,scheduledLaunch: Option[ChangeRecord]= None,embargo: Option[ChangeRecord]= None,expiry: Option[ChangeRecord]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w1.value -> None,w2.value -> None,w3.value -> None,w5.value -> None,w6.value -> None,w7.value -> None,w8.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.lastModified,w2.value -> x.created,w3.value -> x.published,w4.value -> x.revision,w5.value -> x.takenDown,w6.value -> x.scheduledLaunch,w7.value -> x.embargo,w8.value -> x.expiry)
...
} yield ContentChangeDetails(lastModified,created,published,revision,takenDown,scheduledLaunch,embargo,expiry)
}