...
case class Tag(id: Long,`type`: Option[String]= None,internalName: Option[String]= None,externalName: Option[String]= None,slug: Option[String]= None,section: Option[Section]= None,path: Option[String]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w2.value -> None,w3.value -> None,w4.value -> None,w5.value -> None,w6.value -> None,w7.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.id,w2.value -> x.`type`,w3.value -> x.internalName,w4.value -> x.externalName,w5.value -> x.slug,w6.value -> x.section,w7.value -> x.path)
...
} yield Tag(id,`type`,internalName,externalName,slug,section,path)
}