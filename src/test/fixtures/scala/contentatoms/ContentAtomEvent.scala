...
case class ContentAtomEvent(atom: Atom,eventType: EventType,eventCreationTime: DateTime) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.atom,w2.value -> x.eventType,w3.value -> x.eventCreationTime)
...
} yield ContentAtomEvent(atom,eventType,eventCreationTime)
}