...
case class StoryQuestionsAtom(
  relatedStoryId:       String,
  relatedStoryLinkType: RelatedStoryLinkType,
  title:                String,
  editorialQuestions:   Option[List[QuestionSet]]     = None,
  userQuestions:        Option[List[QuestionSet]]     = None,
  notifications:        Option[NotificationProviders] = None,
  closeDate:            Option[DateTime]              = None) extends TStruct
...
object StoryQuestionsAtom extends TStructCodec[StoryQuestionsAtom] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.relatedStoryId,
    w2.value -> x.relatedStoryLinkType,
    w3.value -> x.title,
    w4.value -> x.editorialQuestions,
    w5.value -> x.userQuestions,
    w6.value -> x.notifications,
    w7.value -> x.closeDate)
  ...
  } yield StoryQuestionsAtom(
    relatedStoryId,
    relatedStoryLinkType,
    title,
    editorialQuestions,
    userQuestions,
    notifications,
