...
case class ResultGroup(title: String,share: String,minScore: Short,id: String) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.title,w2.value -> x.share,w3.value -> x.minScore,w4.value -> x.id)
...
} yield ResultGroup(title,share,minScore,id)
}