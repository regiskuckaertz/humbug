...
case class RecipeAtom(title: String,tags: Tags,time: Time,serves: Option[Serves]= None,ingredientsLists: List[IngredientsList],steps: List[String],credits: List[String],images: List[Image],sourceArticleId: Option[String]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w4.value -> None,w9.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.title,w2.value -> x.tags,w3.value -> x.time,w4.value -> x.serves,w5.value -> x.ingredientsLists,w6.value -> x.steps,w7.value -> x.credits,w8.value -> x.images,w9.value -> x.sourceArticleId)
...
} yield RecipeAtom(title,tags,time,serves,ingredientsLists,steps,credits,images,sourceArticleId)
}