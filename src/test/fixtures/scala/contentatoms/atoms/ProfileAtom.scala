...
case class ProfileAtom(typeLabel: Option[String]= None,headshot: Option[Image]= None,items: List[ProfileItem],entity: Option[Entity]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w1.value -> None,w2.value -> None,w4.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.typeLabel,w2.value -> x.headshot,w3.value -> x.items,w4.value -> x.entity)
...
} yield ProfileAtom(typeLabel,headshot,items,entity)
}