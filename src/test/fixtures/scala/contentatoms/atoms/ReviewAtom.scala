...
case class ReviewAtom(reviewType: ReviewType,reviewer: String,rating: Rating,reviewSnippet: String,entityId: String,restaurant: Option[Restaurant]= None,game: Option[Game]= None,film: Option[Film]= None,sourceArticleId: Option[String]= None,images: Option[List[Image]]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.reviewType,w2.value -> x.reviewer,w3.value -> x.rating,w4.value -> x.reviewSnippet,w5.value -> x.entityId,w6.value -> x.restaurant,w7.value -> x.game,w8.value -> x.film,w9.value -> x.sourceArticleId,w10.value -> x.images)
...
} yield ReviewAtom(reviewType,reviewer,rating,reviewSnippet,entityId,restaurant,game,film,sourceArticleId,images)
}