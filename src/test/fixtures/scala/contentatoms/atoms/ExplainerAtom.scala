...
case class ExplainerAtom(title: String,body: String,displayType: DisplayType,tags: Option[List[String]]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.title,w2.value -> x.body,w3.value -> x.displayType,w4.value -> x.tags)
...
} yield ExplainerAtom(title,body,displayType,tags)
}