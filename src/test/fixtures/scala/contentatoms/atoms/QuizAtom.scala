...
case class QuizAtom(id: String,title: String,revealAtEnd: Boolean,published: Boolean,quizType: String,defaultColumns: Option[Short]= None,content: QuizContent) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.id,w2.value -> x.title,w3.value -> x.revealAtEnd,w4.value -> x.published,w5.value -> x.quizType,w6.value -> x.defaultColumns,w7.value -> x.content)
...
} yield QuizAtom(id,title,revealAtEnd,published,quizType,defaultColumns,content)
}