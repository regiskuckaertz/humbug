...
case class MediaAtom(
  assets:                 List[Asset],
  activeVersion:          Option[Version]      = None,
  title:                  String,
  category:               Category,
  plutoProjectId:         Option[String]       = None,
  ...
  byline:                 Option[List[String]] = None,
  commissioningDesks:     Option[List[String]] = None,
  keywords:               Option[List[String]] = None,
  trailImage:             Option[Image]        = None,
  optimisedForWeb:        Option[Boolean]      = None,
  commentsEnabled:        Option[Boolean]      = None,
  suppressRelatedContent: Option[Boolean]      = None) extends TStruct
...
object MediaAtom extends TStructCodec[MediaAtom] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.assets,
    w2.value -> x.activeVersion,
    w3.value -> x.title,
    w4.value -> x.category,
    w5.value -> x.plutoProjectId,
    w6.value -> x.duration,
    w7.value -> x.source,
    w8.value -> x.posterUrl,
    w9.value -> x.description,
    w10.value -> x.metadata,
    w11.value -> x.posterImage,
    w12.value -> x.trailText,
    w13.value -> x.byline,
    w14.value -> x.commissioningDesks,
    w15.value -> x.keywords,
    w16.value -> x.trailImage,
    w17.value -> x.optimisedForWeb,
    w18.value -> x.commentsEnabled,
    w19.value -> x.suppressRelatedContent)
