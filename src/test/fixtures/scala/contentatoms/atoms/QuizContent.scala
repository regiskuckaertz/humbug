...
case class QuizContent(
  ...
  resultGroups:  Option[ResultGroups]  = None,
  resultBuckets: Option[ResultBuckets] = None) extends TStruct
...
object QuizContent extends TStructCodec[QuizContent] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](
    w1.value -> x.questions,
    w2.value -> x.resultGroups,
    w3.value -> x.resultBuckets)
