...
case class GuideAtom(typeLabel: Option[String]= None,guideImage: Option[Image]= None,items: List[GuideItem]) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.typeLabel,w2.value -> x.guideImage,w3.value -> x.items)
...
} yield GuideAtom(typeLabel,guideImage,items)
}