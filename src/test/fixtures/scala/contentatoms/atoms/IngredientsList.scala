...
case class IngredientsList(title: Option[String]= None,ingredients: List[Ingredient]) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.title,w2.value -> x.ingredients)

override def decode = (m) => for {
title <- m.get(w1.value).orElse(defaults.get(w1.value))

ingredients <- m.get(w2.value).orElse(defaults.get(w2.value))
} yield IngredientsList(title,ingredients)
}