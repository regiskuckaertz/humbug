...
case class Serves(`type`: String,from: Short,to: Short,unit: Option[String]= None) extends TStruct
...
override def encode = (x) => HMap[TFieldCodec](w1.value -> x.`type`,w2.value -> x.from,w3.value -> x.to,w4.value -> x.unit)
...
} yield Serves(`type`,from,to,unit)
}