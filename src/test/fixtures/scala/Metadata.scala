...
case class Metadata(tags: Option[List[String]]= None,categoryId: Option[String]= None,license: Option[String]= None,commentsEnabled: Option[Boolean]= None,channelId: Option[String]= None,privacyStatus: Option[PrivacyStatus]= None,expiryDate: Option[DateTime]= None,pluto: Option[PlutoData]= None) extends TStruct
...
override val defaults = HMap[TFieldCodec](w1.value -> None,w2.value -> None,w3.value -> None,w4.value -> None,w5.value -> None,w6.value -> None,w7.value -> None,w8.value -> None)

override def encode = (x) => HMap[TFieldCodec](w1.value -> x.tags,w2.value -> x.categoryId,w3.value -> x.license,w4.value -> x.commentsEnabled,w5.value -> x.channelId,w6.value -> x.privacyStatus,w7.value -> x.expiryDate,w8.value -> x.pluto)
...
} yield Metadata(tags,categoryId,license,commentsEnabled,channelId,privacyStatus,expiryDate,pluto)
}