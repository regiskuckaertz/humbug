...
case class PackagesResponse(status: String, userTier: String, total: Int, startIndex: Int, pageSize: Int, currentPage: Int, pages: Int, orderBy: String, results: List[Package]) extends TStruct

object PackagesResponse extends TStructCodec[PackagesResponse] {
  val w1 = Witness(1)
  ...
  implicit val r9 = new TFieldCodec[w9.T, List[Package]]()
  ...
  } yield PackagesResponse(status, userTier, total, startIndex, pageSize, currentPage, pages, orderBy, results)
}