...
case class CrosswordCreator(name: String, webUrl: String) extends TStruct
...
object CrosswordCreator extends TStructCodec[CrosswordCreator] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.name, w2.value -> x.webUrl)

  override def decode = (m) ⇒ for {
    name ← m.get(w1.value).orElse(defaults.get(w1.value))

    webUrl ← m.get(w2.value).orElse(defaults.get(w2.value))
  } yield CrosswordCreator(name, webUrl)
}