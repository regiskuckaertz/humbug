...
case class Content(id: String, `type`: ContentType = ContentType.ARTICLE, sectionId: Option[String] = None, sectionName: Option[String] = None, webPublicationDate: Option[CapiDateTime] = None, webTitle: String, webUrl: String, apiUrl: String, fields: Option[ContentFields] = None, tags: List[Tag] = List(), elements: Option[List[Element]] = None, references: List[Reference] = List(), isExpired: Option[Boolean] = None, blocks: Option[Blocks] = None, rights: Option[Rights] = None, crossword: Option[Crossword] = None, atoms: Option[Atoms] = None, stats: Option[ContentStats] = None, section: Option[Section] = None, debug: Option[Debug] = None, isGone: Option[Boolean] = None, isHosted: Boolean = `false`, pillarId: Option[String] = None, pillarName: Option[String] = None) extends TStruct
...
object Content extends TStructCodec[Content] {
  ...
  override val defaults = HMap[TFieldCodec](w2.value -> ContentType.ARTICLE, w3.value -> None, w4.value -> None, w5.value -> None, w9.value -> None, w10.value -> List(), w11.value -> None, w12.value -> List(), w13.value -> None, w14.value -> None, w15.value -> None, w16.value -> None, w17.value -> None, w18.value -> None, w19.value -> None, w20.value -> None, w21.value -> None, w22.value -> `false`, w23.value -> None, w24.value -> None)

  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.id, w2.value -> x.`type`, w3.value -> x.sectionId, w4.value -> x.sectionName, w5.value -> x.webPublicationDate, w6.value -> x.webTitle, w7.value -> x.webUrl, w8.value -> x.apiUrl, w9.value -> x.fields, w10.value -> x.tags, w11.value -> x.elements, w12.value -> x.references, w13.value -> x.isExpired, w14.value -> x.blocks, w15.value -> x.rights, w16.value -> x.crossword, w17.value -> x.atoms, w18.value -> x.stats, w19.value -> x.section, w20.value -> x.debug, w21.value -> x.isGone, w22.value -> x.isHosted, w23.value -> x.pillarId, w24.value -> x.pillarName)
  ...
  override def decode = (m) ⇒ for {
    ...
    pillarId ← m.get(w23.value).orElse(defaults.get(w23.value))

    pillarName ← m.get(w24.value).orElse(defaults.get(w24.value))
  } yield Content(id, `type`, sectionId, sectionName, webPublicationDate, webTitle, webUrl, apiUrl, fields, tags, elements, references, isExpired, blocks, rights, crossword, atoms, stats, section, debug, isGone, isHosted, pillarId, pillarName)
}