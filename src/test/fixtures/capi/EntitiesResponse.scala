...
case class EntitiesResponse(status: String, total: Int, results: List[Entity]) extends TStruct

object EntitiesResponse extends TStructCodec[EntitiesResponse] {
  val w1 = Witness(1)
  ...
  implicit val r3 = new TFieldCodec[w3.T, List[Entity]]()
  ...
  } yield EntitiesResponse(status, total, results)
}