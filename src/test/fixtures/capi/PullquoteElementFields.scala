...
case class PullquoteElementFields(html: Option[String] = None, attribution: Option[String] = None) extends TStruct
...
object PullquoteElementFields extends TStructCodec[PullquoteElementFields] {
  ...
  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.html, w2.value -> x.attribution)

  override def decode = (m) ⇒ for {
    html ← m.get(w1.value).orElse(defaults.get(w1.value))

    attribution ← m.get(w2.value).orElse(defaults.get(w2.value))
  } yield PullquoteElementFields(html, attribution)
}