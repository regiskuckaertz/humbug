...
case class VineElementFields(originalUrl: String, title: String, source: String, authorUrl: String, authorUsername: String, html: Option[String] = None, width: Option[Int] = None, height: Option[Int] = None, alt: Option[String] = None, caption: Option[String] = None) extends TStruct

object VineElementFields extends TStructCodec[VineElementFields] {
  val w1 = Witness(1)
  ...
  override val defaults = HMap[TFieldCodec](w6.value -> None, w7.value -> None, w8.value -> None, w9.value -> None, w10.value -> None)

  override def encode = (x) ⇒ HMap[TFieldCodec](w1.value -> x.originalUrl, w2.value -> x.title, w3.value -> x.source, w4.value -> x.authorUrl, w5.value -> x.authorUsername, w6.value -> x.html, w7.value -> x.width, w8.value -> x.height, w9.value -> x.alt, w10.value -> x.caption)
  ...
  override def decode = (m) ⇒ for {
    ...
    alt ← m.get(w9.value).orElse(defaults.get(w9.value))

    caption ← m.get(w10.value).orElse(defaults.get(w10.value))
  } yield VineElementFields(originalUrl, title, source, authorUrl, authorUsername, html, width, height, alt, caption)
}